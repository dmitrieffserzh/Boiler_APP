...
@if(isset($categories[$numb+2]))
    <div class="col-lg-4 my-5 average">
        ...
        <div class="py-3">
            ...
            <a href="{{ route('news.url', implode("/", $categories[$numb+2]->category->ancestorsAndSelf($categories[$numb+2]->category_id)->pluck('slug')->all())) }}" class="small font-weight-bold">{{$categories[$numb+2]->category->title}}</a>
        </div>
    </div>
@endif
