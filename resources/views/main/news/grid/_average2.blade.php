...
@if(isset($categorys[$numb+2]))
    <div class="col-lg-4 my-5 average">
        <img src="{{ '/images/'. $categorys[$numb+2]->image }}" alt="" width="100%">

        <div class="py-3">
            <a class="font-weight-bold text-dark" href="{{ route('news.url', implode("/", $categorys[$numb+2]->category->ancestorsAndSelf($categorys[$numb+2]->category_id)->pluck('slug')->all()).'/'.$categorys[$numb+2]->slug) }}"> {{ $categorys[$numb+2]->title }}</a><br>
            <a href="{{ route('news.url', implode("/", $categorys[$numb+2]->category->ancestorsAndSelf($categorys[$numb+2]->category_id)->pluck('slug')->all())) }}" class="small font-weight-bold">{{$categorys[$numb+2]->category->title}}</a>
        </div>
    </div>
@endif
